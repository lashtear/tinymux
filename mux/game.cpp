...
)
{
    ...
    if (  MuxAlarm.bAlarmed
       || (re = pcre_compile((char *)pattern, PCRE_UTF8|PCRE_NEWLINE_ANYCRLF|case_opt, &errptr, &erroffset, NULL)) == NULL)
    {
	/*
...
static bool check_filter(dbref object, dbref player, int filter, const UTF8 *msg)
{
    ...
    *dp = '\0';
	...
	do
	{
	    ...
	    pcre *re;
	    if (  !MuxAlarm.bAlarmed
	       && (re = pcre_compile((char *)cp, PCRE_UTF8|PCRE_NEWLINE_ANYCRLF|case_opt, &errptr, &erroffset, NULL)) != NULL)
	    {
